...
def calculate_visible_range(scroll_line: int, visible_rows: int, scroll_method: str = "zz") -> tuple[int, int]:
    ...
    for block in highlights:
        ...
        if block_visible:
            ...
            if not block_fits:
                ...
                scroll_method = "zt"
                new_view = calculate_visible_range(scroll_to, visible_rows, scroll_method)
                notes.append(f"Block larger than view, showing from top")
            ...
            directions.append(StageDirection(
                ...
                commands=commands,
                notes=notes if notes else [f"Scroll to center block"]
            ))
...
def format_directions_text(directions: list[StageDirection], rows: int) -> str:
    ...
    for i, d in enumerate(directions, 1):
        ...
        if d.needs_scroll:
            lines.append(f"  Scroll: {d.scroll_to}G{d.scroll_method} → shows lines {d.visible_range[0]}-{d.visible_range[1]}")
        else:
            lines.append(f"  Position: visible in current view (lines {d.visible_range[0]}-{d.visible_range[1]})")
        ...
        if line_stripped.startswith("#"):
            if "@narrate:after" in line_stripped:
                ...
                checkpoints.append(Checkpoint(
                    ...
                    expected_highlight=(visual_start_line, pending_goto) if visual_start_line else None
                ))
            continue
        
        if not line_stripped or line_stripped.startswith("@") or line_stripped.startswith("Set ") or line_stripped.startswith("Output "):
            continue
...
def format_checkpoints_text(checkpoints: list[Checkpoint]) -> str:
    ...
    for i, cp in enumerate(checkpoints, 1):
        ...
        if cp.expected_highlight:
            lines.append(f"  Expected visible: lines {cp.expected_highlight[0]}-{cp.expected_highlight[1]}")
        lines.append("")
